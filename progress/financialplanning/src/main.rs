...
impl From<[u64; 2]> for Investment {
    fn from([this_income, this_cost]: [u64; 2]) -> Self {
        Investment { income: this_income, cost: this_cost }
    }
}
...
fn main() {
    ...
    investments.sort_by(|a, b| {
        a.payoff_time().cmp(&b.payoff_time())
    });

    let mut current_strategy = Investment::default();

    for investment in investments {
        let next_strategy = Investment { income: investment.income + current_strategy.income, cost: investment.cost + current_strategy.cost };

        if cfg!(debug_assertions) {
            println!("Current strategy: payoff: {}, retire: {}", current_strategy.payoff_time(), current_strategy.retire_time(retirement_plan));
            println!("This investment: payoff: {}, retire: {}", investment.payoff_time(), investment.retire_time(retirement_plan));
            println!("Next strategy: payoff: {}, retire: {}", next_strategy.payoff_time(), next_strategy.retire_time(retirement_plan));
            println!("---");
        }
...
fn bad_add<T: std::ops::Add<Output = T>>(a: T, _b: T) -> T {
    a
}
